        ...
        function draw() {
            ...
            function beamdef(y) {
                ...
                for (var i = 0; i <= (beamlength)/5/2+50 ; i++) {
                    ctx.fillRect(i + 25, 4*y*i*i/ (Math.pow((beamlength)/5+100, 3)) * (3 *(beamlength/5+100) - 4*i) - 10 + 210, 1, 20)
                }
                ctx.save();
                ctx.translate(beamlength/5+25+100,0);
                ctx.scale(-1,1);
               for (var i = 0; i <= (beamlength)/5/2+50 ; i++) {
                   ctx.fillRect(i, 4*y*i*i/ (Math.pow((beamlength)/5+100, 3)) * (3 *(beamlength/5+100) - 4*i) - 10 + 210, 1, 20)
               }
                ctx.restore();
                ctx.fillStyle = "black";
        ...
        function plotgraph(){
            ...
            while(i<300){
               ...
               ctx.lineTo(i+20,225-0.9*actdisplace(.003*i)/5);
               ctx.moveTo(i+20,225-0.9*actdisplace(.003*i)/5);
               i+=.01;
           }
          ctx.stroke();
            ctx.beginPath();
